  ...
  return (
    <nav className={`pr-3 ${poppins500.className} block absolute top-[50px]  z-50 right-0 h-screen pt-[70px] pb-5 px-5 pl-[100px] text-right  lg:top-0 lg:pt-0 lg:pb-0 lg:px-0 lg:pl:0 lg:text-start lg:h-fit lg:relative  ${mode ? 'bg-backgroundNight' : 'bg-backgroundWhite'} lg:bg-transparent`}>
      <ul className={`flex leading-[38px] flex-col items-end lg:flex-row lg:items-start`}>
        ...
        <li className={`navLi ${mode ? 'navLiNight hover:text-headerNight' : 'navLiDay hover:text-textWhite'}`}><Link href={"/"}>home</Link></li>
        <li className={`navLi ${mode ? 'navLiNight hover:text-headerNight' : 'navLiDay hover:text-textWhite'}`}><Link href={"/about"}>about</Link></li>
        <li className={`navLi ${mode ? 'navLiNight hover:text-headerNight' : 'navLiDay hover:text-textWhite'}`}>portfolio</li>
        <li className={`navLi ${mode ? 'navLiNight hover:text-headerNight' : 'navLiDay hover:text-textWhite'}`}>service</li>
        <li className={`navLi ${mode ? 'navLiNight hover:text-headerNight' : 'navLiDay hover:text-textWhite'}`}>contact</li>
        {
          user ? <li className={`navLi ${mode ? 'navLiNight hover:text-headerNight' : 'navLiDay hover:text-textWhite'}`} onClick={() => { dispatch(logout()) }} >Logout</li> : <li className={`navLi ${mode ? 'navLiNight hover:text-headerNight' : 'navLiDay hover:text-textWhite'}`}><Link href={"/adminPanel/login"}>Admin</Link></li>
        }
